...
public:
    ...
    bool importStudents(string mapFileName, string setFileName){
        if (trueOrFalse(mapFileName, setFileName)){
            ...
            string setID;
            while (getline(in_fileS, setID)){
                for (int i = 0; i < setID.size(); i++){
                    if (!isdigit(setID[i])) return false;
                }
                unsigned long long int ID = stoi(setID);
                string name, address, number;
                if (!getline(in_fileS, name)) return false;
                if (!getline(in_fileS, address)) return false;
                if (!getline(in_fileS, number)) return false;
                StudentInterface* stu = new Student(ID, name, address, number);
    ...
    bool trueOrFalse(string mapFileName, string setFileName){
        ifstream in_fileM(mapFileName);
        ifstream in_fileS(setFileName);
        if (in_fileM.fail()){
            return false;
